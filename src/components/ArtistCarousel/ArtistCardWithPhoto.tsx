 ...
 * @param index - Card index for color cycling
 */
const ArtistCardWithPhoto: React.FC<ArtistCardWithPhotoProps> = ({ 
  ...
  /**
   * Get DORA color based on card index
   * Cycles through 6 DORA colors using modulo operation
   * @param cardIndex - The index of the card
   * @returns CSS custom property for the color
   */
  const getDoraColor = (cardIndex: number): string => {
    const colors = [
      'var(--dora-pink)',
      'var(--dora-blue)', 
      'var(--dora-orange)',
  ...
  return (
    <motion.div
      ...
      onClick={onClick}
      style={{
        backgroundColor: getDoraColor(index)
      }}
      whileHover={{ 
        scale: 1.03,
        y: -8,
